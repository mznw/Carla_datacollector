...
class LocalPlanner(object):
    ...
    def run_step(self, target_speed=None, debug=False):
        ...
        #print(len(self._waypoint_buffer), 'target', self.target_waypoint)
        def get_next_waypoint(xx):
            if len(xx) > 1:
                return xx[1][0]
            return xx[0][0]
        next_waypoint = get_next_waypoint(self._waypoint_buffer)
        #print('next  ', next_waypoint)
        target_location = (self.target_waypoint.transform.location + next_waypoint.transform.location) / 2.0
        self.target_waypoint = self._map.get_waypoint(target_location)
        #print('new target', self.target_waypoint)
        if target_speed > 50:
