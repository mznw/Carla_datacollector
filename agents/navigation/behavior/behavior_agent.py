...
class BehaviorAgent(Agent):
    ...
    def _overtake(self, location, waypoint, vehicle_list):
        ...
        #print('check for overtake')
        #print('waypoint', waypoint, waypoint.lane_id)
        ...
        #print('left_turn' , left_turn, waypoint.left_lane_marking.type)
        #print('right_turn' , right_turn, waypoint.right_lane_marking.type)
        ...
        if (left_turn == carla.LaneChange.Left or left_turn == carla.LaneChange.Both) \
            ...
            if not new_vehicle_state:
                ...
                #print(left_wpt.transform.location, waypoint.transform.location)
                #print(left_wpt.transform.location - waypoint.transform.location)
                st_location = left_wpt.transform.location + (left_wpt.transform.location - waypoint.transform.location) / 2
                self.set_destination(st_location,
        ...
        if (left_turn == carla.LaneChange.Left or left_turn ==
            ...
            if not new_vehicle_state:
                ...
                print(left_wpt.transform.location, waypoint.transform.location)
                print(left_wpt.transform.location - waypoint.transform.location)
                st_location = left_wpt.transform.location + (left_wpt.transform.location - waypoint.transform.location)
                self.set_destination(st_location,
                                     self.end_waypoint.transform.location, clean=True)
    ...
    def collision_and_car_avoid_manager(self, location, waypoint):
        ...
        vehicle_list = [v for v in vehicle_list if dist(v) < 60 and v.id != self.vehicle.id]
        ...
        if self.direction == RoadOption.CHANGELANELEFT:
            ...
            if vehicle_state and self.direction == RoadOption.LANEFOLLOW and \
                    not waypoint.is_junction and self.speed > 0 \
    ...
    def pedestrian_avoid_manager(self, location, waypoint):
        ...
        if self.direction == RoadOption.CHANGELANELEFT:
            walker_state, walker, distance = self._is_vehicle_hazard(waypoint, location, walker_list, max(
    ...
    def run_step(self, debug=False):
        ...
        if not vehicle_state and self.behavior.overtake_counter != 0:
            self.behavior.overtake_counter = 0
        if vehicle_state and self.behavior.overtake_counter == 0:
            # Distance is computed from the center of the two cars,
            # we use bounding boxes to calculate the actual distance
